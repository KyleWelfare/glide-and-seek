...
@onready var mover: Node2D = $DeAggroRadius/Mover
...
func _process(delta: float) -> void:
	...
	var direction := (target_position - mover.global_position).normalized()

	if not is_chasing:
		if mover.global_position.distance_to(target_position) > 5.0:
			current_speed = min(current_speed + acceleration * delta, move_speed)
		else:
			current_speed = max(current_speed - deceleration * delta, 0.0)

		mover.global_position += direction * current_speed * delta
